    ...
    ,m_eFormat(format)
{
    ...
    m_pData = new UInt8[nHeight * m_nStoreWidth];
...
void Bitmap::Load(const std::string& strFile)
{
    ...
    bmpFile.seekg(m_pFileHeader->bfOffBits,ios::beg);
    bmpFile.read((char*)m_pData,dwSize);
}
...
void Bitmap::Save(const std::string& strFile)
{
    ...
    bmpFile.write((const char*)m_pFileHeader + sizeof(DWORD),BITMAPFILEHEADER::Size() - sizeof(WORD));
    bmpFile.write((const char*)m_pInfoHeader,BITMAPINFOHEADER::Size());
    ...
    bmpFile.write((const char*)m_pData,(dwSize));
}
...
void Bitmap::SetPixel(int nX,int nY,Color color)
{
    ...
    int nStartIndex = nY * m_nStoreWidth + nX * (m_pInfoHeader->biBitCount / 8);
    m_pData[nStartIndex] = color.r;
...
void Bitmap::DrawHLine(int nX,int nX1,int nY,Color color)
{
    ...
    long nStartIndex = nY * m_nStoreWidth + nX * nBits;
...
Color Bitmap::GetPixel(int nX,int nY) const
{
    ...
    long nStartIndex = nY * m_nStoreWidth + nX * (m_pInfoHeader->biBitCount / 8);
    return *((Color*)(&m_pData[nStartIndex]));
}

void Bitmap::Clear(Color clr)
{
    long szColorTable = m_pInfoHeader->biHeight * m_nStoreWidth;
