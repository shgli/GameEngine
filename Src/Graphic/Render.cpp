...
void Render::Draw(RenderList& rendList)
{
	...
	VertexArray* pBuf = rendItem.m_pBuffer;
	for(auto tri : *rendItem.m_vTriangles)
	{
            if(tri->IsVisible())
            {
                VertexIndex& nP0 = tri->GetIndex0();
                VertexIndex& nP1 = tri->GetIndex1();
                VertexIndex& nP2 = tri->GetIndex2();
                m_pTriRender->SetVertices(Vertex(VertexArray::SelectBuffer(*pBuf,nP0),nP0)
                                       ,Vertex(VertexArray::SelectBuffer(*pBuf,nP1),nP1)
                                       ,Vertex(VertexArray::SelectBuffer(*pBuf,nP2),nP2));
