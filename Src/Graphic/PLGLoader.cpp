...
Object* PLGLoader::Load(const std::string & strFile,CoordinateSystem cs)
{
    ...
    //加载顶点列表
    Array<Point> aPositions(nVertices);
    for(Int32 nVertex = 0; nVertex < nVertices; ++nVertex)
    {
	//读取一个顶点
	GetLine(objFile,strStream);
	Point& pt = aPositions[nVertex];
    ...
    VertexArray aVertices(VA_POSITION|VA_COLOR);
    aVertices.SetPositions(aPositions);

    ObjectModelPtr pModel = ObjectModelPtr(new ObjectModel(std::move(aVertices),std::move(aTriangles)));
