...
void RenderTri::SetVertices(const Vertex& v,Int32 nIdx)
{
    ...
    {
	const TextureCoord &coor = v.GetTextureCoord();
...
void RenderTri::RenderLine(Canvas& canvas,const Fragment& start,const Fragment& end,int nY)
{
	...
	if(!canvas.PrepareDraw(nStartX,nY,m_pTexture))
	{
	    return;
	}
        ...
        if(nStartX == nEndX)
        {
            canvas.DrawPixel(nStartX,nY,start,m_pTexture);
        }
        else
        {
            m_lineInter.SetFragment(nStartX,nEndX,start,end);
            for(int x = nStartX; x < nEndX; ++x)
            {
                canvas.DrawPixel(x,nY,m_lineInter.Next(),m_pTexture);
            }
        }
    }
}
