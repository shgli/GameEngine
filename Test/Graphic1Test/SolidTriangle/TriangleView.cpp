...
{
    Super::Init();
    Attributes attris = VA_POSITION|VA_COLOR|VA_TEXTCOOR;
    VertexArray* pBuf = new VertexArray(attris,3);
    *pBuf = {{  0, 50, 0,   255.0,255.0,255.0,0.0, 0.5,1.0}
            ,{-50,-50, 0,   255.0,255.0,255.0,0.0, 0.0,0.0}
            ,{ 50,-50, 0,   255.0,255.0,255.0,0.0, 1.0,0.0}};
    ObjectModel *pModel = new ObjectModel(pBuf);
    pModel->AddTriangle(new Triangle(0,1,2));
    pModel->SetTexture(new Texture("text1.bmp"));
    ...
    //*pBuf = {{0,50,0,255.0,0,255.0}
    //        ,{-50,-50,0,255.0,0,255.0}
    //        ,{50,-50,0,255.0,0,255.0}};
    //ObjectModel *pModel1 = new ObjectModel(pBuf);
    //m_pTriangle1 =  new Object("Triangle1",ObjectModelPtr(pModel));
    //m_pTriangle1->SetCullMode(CM_NEVER);
    //m_pTriangle1->SetBaceFaceMode(BFC_NEVER);
...
//    m_yRot = PI_DIV_4;
   ...
   // (*m_pTriangle1)->LoadIdentity();
   // //(*m_pTriangle)->Scale(0.5,0.5,0.5);
   // //(*m_pTriangle)->Rotate(m_yRot,Vector(0,1,0));
   // (*m_pTriangle1)->Rotate(0,m_yRot,0);
   // (*m_pTriangle1)->Translate(0,0,zTrans);
